...
addpath('F:\code','F:\code\newPackage_020922','F:\code\PhotometryPackage')
% CUSTOMIZE
...
path_to_data = 'F:\PhotomData\NEED TO ANALYZE\vCA1\scent\'; %must end in \
savename = 'grabNE';
...
tankname = 'Avi_stim-210913-102052';% 'Avi_stim-210913-102052';
if details.auto
...
if saveON && saveTXTON
    if details.fearON && ~details.stimON
        save(filename,'dat470','dat405','fit405', 'dfF', 'dfFPerc','Dts','timing', 'f2', 'dataFilt2');
    elseif ~details.fearON && details.stimON
        save(filename,'dat470','dat405','fit405', 'dfF', 'dfFPerc','Dts','timing', 'f2', 'dataFilt2');
    elseif details.fearON && details.stimON
       save(filename,'dat470','dat405','fit405', 'dfF', 'dfFPerc','Dts','timingFear','timingStim', 'f2', 'dataFilt2');
    else
        save(filename,'dat470','dat405','fit405', 'dfF','dfFPerc','Dts', 'f2', 'dataFilt2');
    end  
elseif saveON
    if details.fearON && ~details.stimON
        save(filename,'dat470','dat405','fit405', 'dfF', 'dfFPerc','Dts', 'f2', 'dataFilt2');
...
if saveTXTON
    ...
    for k = 1:length(matFiles)
        ...
        tmp2 = tmp{3}; tmp2 = strsplit(tmp2,'-'); tmp2 = tmp2{1};
        subStr = tmp{1}; region = [tmp{2} '_' tmp2];
        subNumb   = [tmpName(strfind(tmpName, subStr) + length(subStr))]; % will get sub
        load(tmpName)

        timingFile = [subStr '_' region '.txt']; %[subStr 'Stim_times' subNumb '.txt']
